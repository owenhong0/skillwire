...
export default function App() {
  const [view, setView] = useState('skills') // 'skills' | 'repos' | 'learn'
  ...
  const switchView = (v) => {
    if (v === view) return
    setView(v)
    if (v !== 'learn' && q.trim()) search(q, typeFor(v))
  }
    ...
    <>
      ...
      <main>
        <nav className="nav" aria-label="Sections">
          <button aria-current={view === 'skills'} onClick={() => switchView('skills')}>
            Skills
          </button>
          <button aria-current={view === 'repos'} onClick={() => switchView('repos')}>
            Repos
          </button>
          <button aria-current={view === 'learn'} onClick={() => switchView('learn')}>
            Learn
          </button>
        </nav>
        ...
        {view === 'learn' ? (
          ...
          <>
            ...
            <div id="grid">
              ...
              {loading &&
                Array.from({ length: cards.length ? 2 : 4 }).map((_, i) => (
              ...
              {status === 'idle' && (
                <div className="empty">
                  ...
                  <div>
                    Type a function — like <em>pdf</em>, <em>postgres</em>, or{' '}
                    <em>browser automation</em> — and hit Search. Results stream in from real GitHub
                    activity. New to the terms? Open <em>Learn</em>.
                  </div>
                </div>
              )}
            </div>
